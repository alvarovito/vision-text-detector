...
int Component::isSimilar(Component similar){
	...
	assert(stats);
	//similar median
	if ((float)(similar.median) > (float)(median*1.2) || (float)(similar.median) < (float)(median*0.8))
		return 0;

	//similar height
	if ((float)(similar.getHeigth()) > (float)(this->getHeigth()*1.2) || (float)(similar.getHeigth()) < (float)(this->getHeigth()*0.8))
		return 0;

	//similar widht
	/*if ((float)(similar.getWidth()) > (float)(this->getWidth()*1.4) || (float)(similar.getWidth()) < (float)(this->getWidth()*0.6))
		return 0;*/

	//Center distances are higher than 3 times candidate width
	int distance_centers = similar.getCenter().x - this->getCenter().x;
	if (distance_centers > 2 * this->getWidth())
		return 0;
